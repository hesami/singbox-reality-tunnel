...
# Keep menu-driven server management resilient: log failures instead of
# silently terminating on a non-critical command.
...
_print_status_bar() {
    ...
    systemctl is-active --quiet sing-box-grpc     2>/dev/null \
        && vgrpc_svc="${GREEN}●${NC}"  || vgrpc_svc="${DIM}○${NC}"
    echo -e "  ${DIM}Services :${NC}  ${vless_svc} Reality  ${vws_svc} WS  ${vgrpc_svc} gRPC  ${hy2_svc} Hysteria2  ${auth_svc} Sub  ${f2b_svc} F2B"
...
_service_control_menu() {
    while true; do
        ...
        echo ""
        service_status_line sing-box          "VLESS+Reality     (sing-box)"
        service_status_line sing-box-ws       "VLESS+WS+TLS      (sing-box-ws)"
        service_status_line sing-box-client   "Tunnel client     (sing-box-client)"
        service_status_line sing-box-grpc     "VLESS+gRPC+TLS    (sing-box-grpc)"
        ...
        echo -e "  ${CYAN}2)${NC}  VLESS+Reality → start / stop / restart"
        echo -e "  ${CYAN}3)${NC}  VLESS+WS+TLS  → start / stop / restart"
        echo -e "  ${CYAN}4)${NC}  VLESS+gRPC    → start / stop / restart"
        echo -e "  ${CYAN}5)${NC}  Hysteria2     → start / stop / restart"
        echo -e "  ${CYAN}6)${NC}  Auth/Sub API  → start / stop / restart"
        echo -e "  ${CYAN}7)${NC}  Tunnel client → start / stop / restart"
        ...
        case "$MENU_CHOICE" in
            ...
            3) vws_service_menu ;;
            4) vgrpc_service_menu ;;
            5) hy2_service_menu ;;
            6) _simple_service_menu "hysteria-auth" "Auth + Subscription API" ;;
            7) _simple_service_menu "sing-box-client" "Tunnel client" ;;
            0) return ;;
            *) print_warn "Invalid choice."; sleep 1 ;;
        esac
    done
...
_uninstall_menu() {
    while true; do
        ...
        echo -e "  ${CYAN}2)${NC}  Remove VLESS + WS + TLS"
        echo -e "  ${CYAN}3)${NC}  Remove VLESS + gRPC + TLS"
        echo -e "  ${CYAN}4)${NC}  Remove Hysteria2"
        echo -e "  ${CYAN}5)${NC}  ${RED}Full Factory Reset${NC}  ${DIM}(everything)${NC}"
        echo -e "  ${CYAN}0)${NC}  Back"
        menu_prompt
        case "$MENU_CHOICE" in
            1) vless_uninstall ;;
            2) vws_uninstall; press_enter ;;
            3) vgrpc_uninstall; press_enter ;;
            4) hy2_uninstall ;;
            5) _uninstall_all ;;
            0) return ;;
            *) print_warn "Invalid."; sleep 1 ;;
