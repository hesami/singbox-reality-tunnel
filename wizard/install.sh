...
wizard_install() {
    ...
    echo ""
    echo -e "  ${CYAN}1)${NC}  VLESS + Reality      ${DIM}(TCP, stable)${NC}"
    echo -e "  ${CYAN}2)${NC}  Hysteria2            ${DIM}(QUIC/UDP, fast)${NC}"
    echo -e "  ${CYAN}3)${NC}  VLESS + WS + TLS     ${DIM}(CDN-compatible)${NC}"
    echo -e "  ${CYAN}4)${NC}  VLESS + gRPC + TLS   ${DIM}(CDN-compatible, lower latency than WS)${NC}"
    echo -e "  ${CYAN}5)${NC}  All protocols        ${DIM}(recommended — client picks best)${NC}"
    echo -e "  ${CYAN}6)${NC}  Reality + Hysteria2  ${DIM}(no CDN needed)${NC}"
    echo ""
    menu_prompt
    ...
    case "$MENU_CHOICE" in
        ...
        *) install_vless=true; install_hy2=true ;;
    esac
  ...
  \"outbounds\": [{\"type\": \"direct\", \"tag\": \"direct\"}]
    ...
    if $install_grpc; then
        ...
        if [[ -n "$grpc_domain" ]]; then
            local grpc_port grpc_svc grpc_uuid grpc_token
            ask grpc_port "  gRPC listen port" "443"
            grpc_svc=$(openssl rand -hex 8 2>/dev/null || cat /dev/urandom | tr -dc 'a-z0-9' | head -c 16)
            grpc_uuid=$(generate_uuid)
            grpc_token=$(generate_token)
            ! ssl_cert_exists "$grpc_domain" && ssl_install_acme && ssl_issue "$grpc_domain" || true
            vgrpc_write_config "$grpc_port" "$grpc_svc" "$grpc_domain" "$grpc_uuid"
            vgrpc_save_info "$grpc_port" "$grpc_svc" "$grpc_domain"
            vgrpc_create_service
            open_port "$grpc_port" tcp
            service_start "$VGRPC_SERVICE" || print_warn "sing-box-grpc failed to start."
            # Register engine on existing user or create standalone
            if $install_vless || $install_hy2; then
...
PYEOF
    ...
    $install_ws    && echo -e "  ${DIM}• WS+TLS: enable CDN proxy in ArvanCloud after confirming direct connection${NC}"
    $install_grpc  && echo -e "  ${DIM}• gRPC: enable HTTP/2 + gRPC support in ArvanCloud CDN settings${NC}"
    echo ""
    press_enter
}
